...
void Scene::AddModel(std::string path, VkQueue graphicsQueue, VkCommandPool graphicsCommandPool, VkPhysicalDevice physicalDevice, VkDevice logicalDevice)
{
    // -- temporary --
    Texture tex;
    ...
    {//if material had no texture, set 0 to indicate no texture, texture 0 will be reserved for a default texture
        ...
        {// otherwise, create texture and set value to index of new texture
            Texture texture;
            matToTex[i] = texture.createTexture(textureNames[i], graphicsQueue, graphicsCommandPool, physicalDevice, logicalDevice);
